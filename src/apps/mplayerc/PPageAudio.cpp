...
void CPPageAudio::OnHScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar)
{
	if (*pScrollBar == m_volumectrl) {
		...
		AfxGetMainFrame()->m_wndToolBar.m_volctrl.SetRangeFromBoost(m_nVolumeBoost);
	}
...
void CPPageAudio::OnCancel()
{
	...
	if (m_nVolumeBoost != m_oldVolumeBoost) {
		...
		AfxGetMainFrame()->m_wndToolBar.m_volctrl.SetRangeFromBoost(m_oldVolumeBoost);
	}
