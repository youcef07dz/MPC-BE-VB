...
void CAppSettings::LoadSettings(bool bForce/* = false*/)
{
	...
	profile.ReadInt(IDS_R_SETTINGS, IDS_RS_VOLUMEBOOST, nVolumeBoost, 1, 10);
	...
	// Audio
	profile.ReadInt(IDS_R_AUDIO, IDS_RS_VOLUME, nVolume, 0, 1000);
	...
	while (it != cmdln.end()) {
		...
		if ((param[0] == '-' || param[0] == '/') && param.GetLength() > 1) {
			...
			if (sw == L"open") {
				...
				if (volumeValue >= 0 && volumeValue <= 1000) {
